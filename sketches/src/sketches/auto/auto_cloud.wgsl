...
fn cloud_tunnel(pos: vec2f) -> vec3f {
    ...
    loop {
        ...
        let len_pxy = length(p.xy);
        n = len_pxy - 1.0;
        step = 1.5 - len_pxy - n * 0.3;

        // 5 fixed octaves (was ~7 iteration while loop)
        step -= abs(dot(sin(p * 4.875), vec3f(detail))) / 0.075;
        step -= abs(dot(sin(p * 8.32), vec3f(detail))) / 0.128;
        step -= abs(dot(sin(p * 14.235), vec3f(detail))) / 0.219;
        step -= abs(dot(sin(p * 24.31), vec3f(detail))) / 0.374;
        step -= abs(dot(sin(p * 41.47), vec3f(detail))) / 0.638;

        let cloud_depth = smoothstep(0.0, 20.0, d);
        let center_factor = smoothstep(0.0, 10.5, len_pxy);
