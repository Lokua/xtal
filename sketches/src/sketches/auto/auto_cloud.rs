...
pub fn init(app: &App, ctx: &Context) -> AutoCloud {
    let wr = ctx.window_rect();

    let hub = ControlHub::from_path(
        to_absolute_path(file!(), "auto_cloud.yaml"),
        Timing::new(ctx.bpm()),
    );
    ...
    let gpu = gpu::GpuState::new_fullscreen(
        app,
        wr.resolution_u32(),
        to_absolute_path(file!(), "auto_cloud.wgsl"),
        &params,
        0,
    );
