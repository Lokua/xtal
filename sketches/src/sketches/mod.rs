pub mod blob;
pub mod cloud_tunnel;
pub mod d_warp;
pub mod displacement_2a;
pub mod drop;
pub mod drop_tines;
pub mod drop_walk;
pub mod dynamic_uniforms;
pub mod g_warp;
pub mod grid_splash;
pub mod heat_mask;
pub mod interference;
pub mod kalos;
pub mod kalos_2;
pub mod marcher;
pub mod neon;
pub mod neural;
pub mod ray_marching;
pub mod shaxper;
pub mod spiral;
pub mod spiral_lines;
pub mod swirl;
pub mod symmetry;
pub mod toxy;
pub mod wave_fract;

pub mod auto;
pub use self::auto::auto_acc;
pub use self::auto::auto_cloud;
pub use self::auto::auto_dreams;
pub use self::auto::auto_dyn_uni;
pub use self::auto::auto_flow;
pub use self::auto::auto_grid_splash;
pub use self::auto::auto_grid_splash_bw;
pub use self::auto::auto_gyroid;
pub use self::auto::auto_hatch;
pub use self::auto::auto_ink;
pub use self::auto::auto_rm;
pub use self::auto::auto_spiral;
pub use self::auto::auto_sline;
pub use self::auto::auto_un;
pub use self::auto::auto_wave_fract;

pub mod dev;
pub use self::dev::animation_dev;
pub use self::dev::audio_controls_dev;
pub use self::dev::audio_dev;
pub use self::dev::bug_repro;
pub use self::dev::control_script_dev;
pub use self::dev::crosshatch_dev;
pub use self::dev::cv_dev;
pub use self::dev::effects_wavefolder_dev;
pub use self::dev::midi_dev;
pub use self::dev::non_yaml_dev;
pub use self::dev::osc_dev;
pub use self::dev::osc_transport_dev;
pub use self::dev::recording_dev;
pub use self::dev::responsive_dev;
pub use self::dev::shader_to_texture_dev;
pub use self::dev::wgpu_compute_dev;

pub mod genuary_2025;
pub use self::genuary_2025::g25_1_horiz_vert;
pub use self::genuary_2025::g25_2_layers;
pub use self::genuary_2025::g25_5_isometric;
pub use self::genuary_2025::g25_10_11_12;
pub use self::genuary_2025::g25_13_triangle;
pub use self::genuary_2025::g25_14_black_and_white;
pub use self::genuary_2025::g25_18_wind;
pub use self::genuary_2025::g25_19_op_art;
pub use self::genuary_2025::g25_20_23_brutal_arch;
pub use self::genuary_2025::g25_22_gradients_only;

pub mod genuary_2026;
pub use self::genuary_2026::g26_11_quine;
pub use self::genuary_2026::g26_12_boxes;
pub use self::genuary_2026::g26_13_portrait;
pub use self::genuary_2026::g26_14_dreams;
pub use self::genuary_2026::g26_15;

pub mod scratch;
pub use self::scratch::basics;
pub use self::scratch::bos;
pub use self::scratch::breakpoints_2;
pub use self::scratch::brutalism;
pub use self::scratch::chromatic_aberration;
pub use self::scratch::displacement_1;
pub use self::scratch::displacement_1a;
pub use self::scratch::displacement_2;
pub use self::scratch::easing_vis;
pub use self::scratch::fb;
pub use self::scratch::flow_field_basic;
pub use self::scratch::lines;
pub use self::scratch::noise;
pub use self::scratch::perlin_loop;
pub use self::scratch::rm_rep;
pub use self::scratch::sand_line;
pub use self::scratch::sand_lines;
pub use self::scratch::sierpinski_triangle;
pub use self::scratch::unt;
pub use self::scratch::vertical;
pub use self::scratch::vertical_2;
pub use self::scratch::z_sim;

pub mod common;

pub mod templates;
pub use self::templates::basic_cube_shader_template;
pub use self::templates::du_fs_template;
pub use self::templates::du_fs_texture_template;
pub use self::templates::fullscreen_shader_template;
pub use self::templates::template;
