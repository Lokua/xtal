...
struct Params {
    ...
    // offset_mult_10, unused, unused, unused
    b: vec4f,
    ...
    // animate_wave_phase, invert_animate_wave_phase,
    // animate_stripe_phase, invert_animate_stripe_phase
    j: vec4f,

    // animate_steep_phase, invert_animate_steep_phase,
    // animate_quant_phase, invert_animate_quant_phase
    k: vec4f,
}
...
fn vs_main(@builtin(vertex_index) vidx: u32) -> VertexOutput {
    ...
    let points_per_segment = max(params.c.x, 2.0);
    let noise_scale = params.c.y;
    let angle_variation = params.c.z;
    let n_lines = max(params.c.w, 2.0);
    ...
    // Legacy auto_spiral hardcoded a horizontal line axis via Rust-side
    // uniform overrides (b1..b4). In v2 we keep that axis explicit here.
    let line_dir = vec2f(1.0, 0.0);
...
fn get_circle_pos(
    ...
    let offset_mult = select(params.d.w, params.d.w * 10.0, params.b.x >= 0.5);
...
fn apply_stripe_modulation(radius: f32, pos_angle: f32) -> f32 {
    ...
    let stripe_phase = compute_phase(
        params.i.y,
        params.j.z,
        params.j.w,
        224.0,
...
fn apply_quant_modulation(radius: f32, pos_angle: f32) -> f32 {
    ...
    let quant_freq = max(params.h.y, 0.0001);
    let quant_phase = compute_phase(
        params.h.z,
        params.k.z,
        params.k.w,
        96.0,
...
fn compute_phase(
    ...
    let total_beats = animation_time * phase_animation_mult * global_slowdown;
    if (total_beats <= 0.0001) {
        return base_phase;
    }

    let beats = max(params.a.z, 0.0);
    let ramp = fract(beats / total_beats);
