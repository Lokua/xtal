...
Use in wgsl like so (assuming `params` uniform):

```wgsl
let checkbox_example = bool(params.checkbox_example);
```
...
- `stem` - optional. A seed value used to differentiate this animation from
  ...
  (e.g. 100, 101) as the internal seed formula only shifts by 1 per loop cycle,
  which can produce correlated output between nearby stems.
...
- `stem` - optional. A unique ID for internal slew state. When omitted, a
  ...
  sequential values (e.g. 100, 101) as the internal seed formula only shifts by
  1 per loop cycle, which can produce correlated output between nearby stems.
...
- An automatic final `kind: end` is generated at `snapshots.len() * beats`.
